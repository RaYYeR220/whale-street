...
export function registerWs(app: FastifyInstance, e: Engine): void {
  ...
  const leaderboard = (): ServerMessage => ({ t: 'leaderboard', rows: e.exchange.leaderboard(50) });
  ...
  app.get('/ws', { websocket: true }, (socket) => {
    const c: Client = { socket, channels: new Set(), playerId: null, alive: true };
    clients.add(c);
    ...
    socket.on('message', (raw) => {
      ...
      for (const ch of msg.channels) {
        ...
        if (ch === 'leaderboard') send(c, leaderboard());
